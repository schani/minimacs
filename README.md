...
Press `C-h b` (Emacs' `describe-bindings`) to open a read-only `*Help*`
buffer containing every binding. That list is generated by walking minimacs'
active keymap rather than maintained separately.
...
| Key | Action |
|-----|--------|
| `C-h b` | Show the machine-generated keybinding list in a read-only buffer |
